...
<CLColorPickerViewDelegate, CLFontPickerViewDelegate, UITextViewDelegate, UITextFieldDelegate>
...
- (void)customInit
{
    ...
    _scrollView.backgroundColor = [UIColor colorWithRed:72.0/255.0 green:72.0/255.0 blue:72.0/255.0 alpha:1.0];
    [self addSubview:_scrollView];
    
    _textView = [[UITextView alloc] initWithFrame:CGRectMake(10, 0, self.width-42, 80)];
    _textView.delegate = self;
    _textView.backgroundColor = [UIColor colorWithRed:72.0/255.0 green:72.0/255.0 blue:72.0/255.0 alpha:1.0];
    ...
    _colorPanel.backgroundColor = [UIColor colorWithRed:72.0/255.0 green:72.0/255.0 blue:72.0/255.0 alpha:1.0];
...
- (void)textViewDidChange:(UITextView*)textView
{
    ...
    if(selection.location+selection.length == textView.text.length && [textView.text characterAtIndex:textView.text.length-1] == '\n') {
        ...
        //V
        NSString* subString = [textView.text substringToIndex:textView.text.length - 1];
        ...
        if (self.parentView != nil) {
            self.parentView.bottomTextLabel.text = subString;
            [self.parentView pushedButton:nil];
        }
    } else if (selection.location+selection.length == textView.text.length && textView.text.length > 25) {
        //V
        NSString* subString = [textView.text substringToIndex:textView.text.length - 1];
        textView.text = subString;
    }
    ...
    //V
    if (self.parentView != nil) {
        self.parentView.bottomTextLabel.text = textView.text;
//        [self.parentView pushedButton:nil];
    }
